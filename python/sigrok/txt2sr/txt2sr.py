...
from np2srzip.np2srzip import np2srzip   # change to your np2srzip location
...
class SRZipExporterApp:
    def __init__(self, root):
        ...
        self.export_btn = tk.Button(button_frame, text="Export SRZip", command=self.export_sr)
        self.export_btn.pack(side="left", padx=5)

        tk.Button(button_frame, text="Open PulseView", command=self.open_pulseview).pack(side="left", padx=5)

        # auto-open checkbox
        self.auto_open_var = tk.BooleanVar(value=False)
        tk.Checkbutton(button_frame, text="Auto-open in PulseView", variable=self.auto_open_var).pack(side="left", padx=10)
        ...
        self.log_area = scrolledtext.ScrolledText(log_frame, width=120, height=10, state="disabled")
    ...
    def browse_output(self):
        file = filedialog.asksaveasfilename(defaultextension=".sr",
